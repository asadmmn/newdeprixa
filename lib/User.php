...
function cdp_getAddresses($id)
{
    $db = new Conexion;

    $db->cdp_query('SELECT * FROM cdb_senders_addresses WHERE user_id=:id');
    ...
    public function cdp_getcountry($phone)
    {

        var_dump($phone);
        die();
                                // Extract phone code (first 3 characters for each user)
                                $userphonecode = substr($phone, 0, 3);

                                // If the phone code starts with a "+", we remove it to check for both cases (with and without +)
                                $cleanedPhoneCode = ltrim($userphonecode, '+');  // Removes '+' if present
        
                                // Prepare the query to match the phone code in both formats (with and without '+')
                                $this->db->cdp_query("
                                    SELECT name 
                                    FROM cdb_countries 
                                    WHERE phone_code = :exact_code OR phone_code = :alternate_code
                                ");
        
                                // Bind both the exact code (with or without '+') and alternate code (with/without '+')
                                $this->db->bind(':exact_code', $userphonecode);  // Original code from user (with or without '+')
                                $this->db->bind(':alternate_code', $cleanedPhoneCode);  // Code without '+'
        
                                // Execute the query for the current user
                                $this->db->cdp_execute();
        
                                // Fetch the country result
                                $country = $this->db->cdp_registro();
